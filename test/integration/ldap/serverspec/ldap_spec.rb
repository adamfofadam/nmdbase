...
describe file('/etc/ldap.conf') do
  ...
  its(:content) { should match '^base dc=ldap,dc=example,dc=com$' }
  its(:content) { should match '^uri ldap://ldap.example.com/$' }
  its(:content) { should match '^ldap_version 3$' }
  its(:content) { should match '^rootbinddn cn=admin,dc=ldap,dc=example,dc=com$' }
