...
describe "nmdbase::yubico" do
  ...
  it "Creates a global yubico auth file." do
    ...
    expect(chef_run).to render_file('/etc/yubikey_mappings').with_content(/^test_user:cccccexample$/)
  end
  ...
  it "Configures the sshd PAM module." do
    ...
    expect(chef_run).to render_file(path).with_content(/^# This file was generated by Chef for*.+$/)
    expect(chef_run).to render_file(path).with_content(/^# Do NOT modify this file by hand!$/)
    expect(chef_run).to render_file(path).with_content(%r{^auth required pam_yubico.so mode=client try_first_pass authfile=/etc/yubikey_mappings debug})
    expect(chef_run).to render_file(path).with_content(/^@include common-auth$/)
    expect(chef_run).to render_file(path).with_content(/^@include common-account$/)
    expect(chef_run).to render_file(path).with_content(/^@include common-session$/)
    expect(chef_run).to render_file(path).with_content(/^session +optional +pam_motd.so # \[1\]$/)
    expect(chef_run).to render_file(path).with_content(/^account +required +pam_nologin.so$/)
    expect(chef_run).to render_file(path).with_content(/^session +optional +pam_motd.so # \[1\]$/)
    expect(chef_run).to render_file(path).with_content(/^session +optional +pam_mail.so standard noenv # \[1\]$/)
    expect(chef_run).to render_file(path).with_content(/^session +required +pam_limits.so$/)
    expect(chef_run).to render_file(path).with_content(/^session +required +pam_env.so # \[1\]$/)
    expect(chef_run).to render_file(path).with_content(%r{^session +required +pam_env.so +user_readenv=1 envfile=/etc/default/locale$})
    expect(chef_run).to render_file(path).with_content(/^@include common-password$/)
  end
