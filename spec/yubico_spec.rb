...
describe 'nmdbase::yubico - Ubuntu tests', :ubuntu do
  ...
  it 'Configures the sshd PAM module.' do
    ...
    expect(chef_run).to render_file(path)
      .with_content(%r{^auth\srequired\spam_yubico.so\smode=client\s
                    try_first_pass\sauthfile=\/etc\/yubikey_mappings\sdebug}x)
    expect(chef_run).to render_file(path)
    ...
    expect(chef_run).to render_file(path)
      .with_content(%r{session\srequired\spam_env.so\suser_readenv=1\s
