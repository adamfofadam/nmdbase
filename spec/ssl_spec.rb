...
describe "nmdbase::ssl" do
  ...
  it "Installs the client key." do
    ...
    expect(chef_run).to create_file('/etc/ssl/private/example.key').with_content(/^test_key$/)
  end
  ...
  it "Installs the client cert." do
    ...
    expect(chef_run).to create_file('/etc/ssl/certs/example.crt').with_content(/^test_crt$/)
  end
