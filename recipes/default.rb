...
%w(epel-release ntp vim-X11 vim-common vim-enhanced vim-minimal nano nc telnet cyrus-sasl-plain wget python-pip).each do |pkg|
  package pkg do
    action :upgrade
  end
end
...
cookbook_file "/bin/s3download" do
  owner 'root'
  group 'root'
  mode '0755'
  action :create
